  ...
  const RolesPermissionsTable = () => {
    return (
      <div dir="rtl" className=" space-y-4">
        {/* عنوان الأعمدة */}
        <div className="grid grid-cols-5 bg-white/20 text-white font-bold text-sm p-2 rounded-md">
          <div>م</div>
          <div>الإسم</div>
          <div>المستخدمين</div>
        ...
        {roles.map((role, idx) => (
          <div
            key={role.id}
            className="grid grid-cols-5 items-center bg-white/10 backdrop-blur-md rounded-xl text-white p-2 shadow-sm hover:shadow-md transition-all duration-200"
          >
            <div className="text-sm font-semibold">{idx + 1}</div>
            <div className="text-sm">{role.name}</div>
            <div className="text-sm">
              <div className="w-fit px-2 py-1 bg-white/20  flex items-center cursor-pointer rounded-xl">
            ...
            <div className="flex justify-center gap-2">
              <button className="bg-white/20 hover:bg-white/40 text-white px-3 py-1 rounded-md text-sm">
                  <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor" className="size-6">
